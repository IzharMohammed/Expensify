...
export function Card({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn('rounded-2xl border border-border/55 bg-card text-card-foreground shadow-soft transition-[box-shadow,border-color,transform] duration-300', className)}
      {...props}
    />
  );
}
